...
format:
  clean-revealjs:
    ...
    code-overflow: wrap
    footer: "[Introduction to SQL](https://danilofreire.github.io/datasci151-summer/lectures/lecture-11/11-introduction-sql.html)"
transition: slide
...
# Hey, there! 😊 <br> I hope you're all doing well! {background-color="#4338CA"}

# Today's lecture 📚 {background-color="#4338CA"}
...
# Structured Query Language (SQL) {background-color="#4338CA"}
...
# Let's get started! 🤓 {background-color="#4338CA"}
...
# Any issues with the installation? 🤔 {background-color="#4338CA"}

# Let's create our first SQL table! 🚀 {background-color="#4338CA"}
...
- Five core storage classes:
  - [`TEXT`]{.alert}: strings (UTF-8)
  - [`INTEGER`]{.alert}: whole numbers
  - [`REAL`]{.alert}: decimals (floating-point, IEEE 754, 8 bytes)
  - [`BLOB`]{.alert}: binary data, stored as-is
  - [`NULL`]{.alert}: missing or undefined values
- Plus type affinities like `NUMERIC` and convenience names like `BOOLEAN`, `DATE`, `TIME`, `DATETIME`
:::
...
- `ROWID` is a hidden auto-generated column, [but avoid it]{.alert}: its values can be reused for new rows after deletions (so a stale reference may point at a different row) and the column is not portable across SQL databases
:::
:::
:::
...
# Table Schemas 📄 {background-color="#4338CA"}
...
# Table Operations 📊 {background-color="#4338CA"}
...
- From the command line:
  - Export to CSV: `sqlite3 -cmd ".mode csv" -cmd ".output example.csv" example.db "SELECT * FROM drivers;"` ([guide](https://www.sqlitetutorial.net/sqlite-export-csv/){data-modal-type="iframe"})
  - Import from CSV: `sqlite3 -cmd ".mode csv" example.db ".import example.csv drivers"` ([guide](https://www.sqlitetutorial.net/sqlite-import-csv/){data-modal-type="iframe"})
:::
...
# And that's it for today! 🚀 {background-color="#4338CA"}

# See you next time! 😊 {background-color="#4338CA"}
