...
subtitle: Lecture 12 - SQLite in Python
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    ...
    code-overflow: wrap
    footer: "[SQLite in Python](https://danilofreire.github.io/datasci151-summer/lectures/lecture-12/12-sql-python.html)"
transition: slide
...
# Hello! Great to see you again! 😊 {background-color="#4338CA"}

# Lecture overview 📚 {background-color="#4338CA"}
...
# Connecting SQLite with Python 🐍 {background-color="#4338CA"}

## Connecting and creating a cursor

:::{style="margin-top: 50px; font-size: 22px;"}
- To interact with an SQLite database from Python, we first need to [establish a connection]{.alert} and then [create a cursor]{.alert}
...
# Executing SQL in Python {background-color="#4338CA"}

## Creating tables

:::{style="margin-top: 50px; font-size: 22px;"}
- We define the structure of our data using [`CREATE TABLE`]{.alert}
...
cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='drivers';").fetchone()
```
:::
...
:::{style="margin-top: 50px; font-size: 22px;"}
- Once the table exists, we populate it using [`INSERT INTO`]{.alert}
...
# Filtering data {background-color="#4338CA"}
...
#| echo: true
#| eval: true
cur.execute("SELECT driver_name, victories FROM drivers WHERE victories NOT BETWEEN 10 AND 60").fetchall()
```
:::
...
:::{style="margin-top: 50px; font-size: 22px;"}
- SQLite `LIKE` is case-insensitive by default [for ASCII letters only]{.alert} (so `'a'` matches `'A'`, but `'é'` does not match `'É'`). Use `LOWER()`/`UPPER()` for [reliable, explicit case handling]{.alert}
...
- For `=`, `<>`, `<`, `>`, `ORDER BY`, and `GROUP BY`, use [`COLLATE NOCASE`]{.alert} to make the comparison case-insensitive. [Note: `COLLATE NOCASE` has no effect on `LIKE`, which uses its own case-folding (toggled with `PRAGMA case_sensitive_like`)]{.alert}

```{python}
#| echo: true
#| eval: true
# Without COLLATE NOCASE, this returns nothing (stored value is 'Mercedes', not 'mercedes')
cur.execute("SELECT driver_name, team FROM drivers WHERE team = 'mercedes' COLLATE NOCASE")
for row in cur.fetchall(): print(row)
```
...
#| echo: true
#| eval: true
# Names that do not start with 'L'
cur.execute("SELECT driver_name FROM drivers WHERE driver_name NOT LIKE 'L%'")
for row in cur.fetchall(): print(row)
```
...
# Handling missing data {background-color="#4338CA"}
...
# Window functions {background-color="#4338CA"}
...
#| echo: true
#| eval: true
# Add more data for better examples (delete first so re-running is safe)
cur.execute("DELETE FROM drivers WHERE driver_name IN ('Valtteri Bottas', 'Sergio Perez', 'Lando Norris', 'Esteban Ocon');")
...
print([col[0] for col in cur.description]) # Print column names
for row in cur.fetchall(): print(row)
```
:::
...
# String manipulation {background-color="#4338CA"}
...
# Conditional logic: `CASE` {background-color="#4338CA"}
...
SELECT driver_name, victories,
    ...
    END AS status
FROM drivers ORDER BY victories DESC;
'''
cur.execute(query_case2)
for row in cur.fetchall(): print(row)
```

- Note: in SQLite, `ORDER BY ... DESC` already places `NULL`s at the end (`NULL` is treated as smaller than any value), so an explicit `NULLS LAST` is redundant here. Other databases (e.g. PostgreSQL) behave differently, so `NULLS LAST` exists for portability
:::
...
# Using SQL with `pandas` 🐼 {background-color="#4338CA"}
...
:::{style="margin-top: 50px; font-size: 22px;"}
*Using standard `sqlite3` fetching:*

```{python}
#| echo: true
#| eval: true
import sqlite3
db_file = 'lecture12.db'
conn = sqlite3.connect(db_file)
cur = conn.cursor()
...
CREATE TABLE employees (
    ...
    name TEXT, dept TEXT, salary INTEGER
);''')
...
avg_salary_dept = df_employees.groupby('dept')['salary'].mean()
