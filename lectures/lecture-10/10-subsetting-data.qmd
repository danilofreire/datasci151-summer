...
format:
  clean-revealjs:
    ...
    footer: "[Subsetting Data](https://danilofreire.github.io/datasci151-summer/lectures/lecture-10/10-subsetting-data.html)"
revealjs-plugins:
  - multimodal
transition: slide
...
# I hope you're all having a great day! 😊 {background-color="#4338CA"}
...
- We will look into five ways to subset data
...
[![](figures/pandas.png)](#){data-modal-type="image" data-modal-url="figures/pandas.png"}
:::
:::
:::
:::
...
[![](figures/tip.png){width="50%"}](#){data-modal-type="image" data-modal-url="figures/tip.png"}

Source: <https://www.reddit.com/r/ProgrammerHumor/comments/1tm1n5q/isthatreallythetruth/>
:::
...
- Especially handy when working with an [unfamiliar dataset]{.alert}
- Uses a [sandboxed environment]{.alert}: changes do not touch the original file unless you save them
...
[![](figures/data-wrangler01.png)](#){data-modal-type="image" data-modal-url="figures/data-wrangler01.png"}
:::
...
# Any questions so far? 🤔 {background-color="#4338CA"}
...
:::{.column width="50%"}
:::{style="text-align: center;"}
[![](figures/data-wrangler08.png)](#){data-modal-type="image" data-modal-url="figures/data-wrangler08.png"}
:::

- Best part: `Data Wrangler` shows the equivalent `pandas` code as you click, so it doubles as a [pandas tutor]{.alert} 🐼
...
- Filter to cars with [6 or more `cylinders`]{.alert}
...
- The [`[]` operator]{.alert} is the most common way to subset data in `pandas`
- Use it to [select a column by name]{.alert}: `df['column']`
- Don't remember the column names? Check them with the [`.columns`]{.alert} attribute
:::
...
- An explanation for this behaviour is that `pandas` is trying to avoid ambiguity
...
- Use the `[]` operator to extract the [`weight` and `acceleration`]{.alert} columns into a new DataFrame
- Remember: selecting [multiple columns]{.alert} needs a list of names inside `[]`, so you'll end up with [two pairs of brackets]{.alert} (`df[[...]]`)
...
- The [inner expression]{.alert} `carfeatures['mpg'] >= 25` builds a [True/False mask]{.alert}, one value per row
- The [outer `[]`]{.alert} then keeps the rows where the mask is `True`
:::
:::
:::
...
## Filtering rows [and]{.alert} selecting columns

:::{style="margin-top: 30px; font-size: 22px;"}
:::{.columns}
:::{.column width="50%"}
- So far the filter has returned [every column]{.alert} for the matching rows
- What if we only care about a [few columns]{.alert}?
- We can chain two `[]` operations: first filter the rows, then pick the columns

```{python}
#| echo: true
#| eval: true
carfeatures[carfeatures['mpg'] >= 25][['mpg', 'cylinders']]
```
:::

:::{.column width="50%"}
- Read it as two steps glued together:
  - `carfeatures[carfeatures['mpg'] >= 25]` keeps the rows with `mpg >= 25`
  - `[['mpg', 'cylinders']]` picks the two columns from that result
- If the line feels crowded, save the filter to a variable first

```{python}
#| echo: true
#| eval: true
fast_cars = carfeatures[carfeatures['mpg'] >= 25]
fast_cars[['mpg', 'cylinders']]
```
:::
:::
:::

## Doing both in one step with `.loc[]`

:::{style="margin-top: 30px; font-size: 21px;"}
:::{.columns}
:::{.column width="50%"}
- The chain works, but `pandas` gives us a [cleaner one-liner]{.alert}: `.loc[]`
- Syntax: `df.loc[row_selector, column_selector]`
- The [row selector]{.alert} can be a boolean mask (just like with `[]`)
- The [column selector]{.alert} is a list of column names

```{python}
#| echo: true
#| eval: true
carfeatures.loc[carfeatures['mpg'] >= 25, ['mpg', 'cylinders']]
```
:::

:::{.column width="50%"}
- Read it as: "from `carfeatures`, take the [rows]{.alert} where `mpg >= 25` and the [columns]{.alert} `mpg` and `cylinders`"
- Multiple conditions work the same way as with `[]`

```{python}
#| echo: true
#| eval: true
carfeatures.loc[
    (carfeatures['cylinders'] == 8) & (carfeatures['mpg'] <= 15),
    ['mpg', 'cylinders', 'acceleration']
]
```

- Why is this useful? `.loc[]` puts [rows and columns side by side]{.alert}, which mirrors how we think about a table
:::
:::
:::
...
We use `.iloc[]` to select data by [row and column position]{.alert}:
...
Remember: [Python is zero-indexed and the last index is not included in lists]{.alert}.
:::
...
- It allows you to filter data using a [string expression, similar to natural language and SQL]{.alert}
...
- Inside `.query()`, `and` and `&` are equivalent (same for `or` and `|`)
- Outside `.query()`, you must use `&` and `|`, because Python's `and`/`or` don't work on pandas Series
...
- Inside `.query()`, both `&` and `and` work; we'll stick with `&` for consistency, but feel free to mix them as you wish
- Parentheses around each condition are not required, but they help readability
```{python}
#| echo: true
#| eval: true
carfeatures.query("(cylinders == 8) & (mpg <= 15) & (acceleration <= 12)")[["mpg", "cylinders", "acceleration"]]
```
:::
