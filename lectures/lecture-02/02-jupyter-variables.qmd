...
format:
  clean-revealjs:
    ...
    footer: "[Packages, Variables, and Lists](https://danilofreire.github.io/datasci151-summer/lectures/lecture-02/02-jupyter-variables.html)"
    slide-number: true
revealjs-plugins:
  - multimodal
transition: slide
...
# Nice to see you all again! 😊 {background-color="#4338CA"}
...
# Python environments {background-color="#4338CA"}
...
#| echo: true
#| eval: false
# Run this in the terminal, not in a Jupyter notebook cell
# Create a new environment called datasci151
conda create -n datasci151 python=3.12 -y
...
conda remove -n datasci151 --all -y
# or: conda env remove -n datasci151 -y
```
...
# Python packages 📦 {background-color="#4338CA"}
...
# Let's start coding: importing packages {background-color="#4338CA"}
...
carfeatures.info() # Check the structure of the DataFrame
```
:::
...
# Running basic analyses {background-color="#4338CA"}
...
carfeatures.cylinders.head()
```
- **Note:** [Dot notation is less flexible than bracket notation]{.alert}, especially for column names with spaces or special characters
:::
:::
:::
...
:::{.callout-note}
The `horsepower` column might be missing from `.describe()` if it was read as non-numeric (e.g., due to placeholder characters like '?'). Data cleaning would be needed to convert it. You can use `carfeatures['horsepower'] = pd.to_numeric(carfeatures['horsepower'], errors='coerce')` to convert it to numeric, which will turn non-convertible values into NaN (missing values).
:::
:::

# Python variables & data types 📊 {background-color="#4338CA"}
...
# Python lists 📝 {background-color="#4338CA"}
...
#| echo: true
#| eval: true
nested_list_example = [[1, 2], ["a", "b"], ["Python", 3.11, 42, True]]
...
# Visualising data with matplotlib 📊 {background-color="#4338CA"}
...
# Any questions? 🤔 {background-color="#4338CA"}

# And that's all for today! 🎉 {background-color="#4338CA"}

# Appendices: Solutions to exercises {background-color="#4338CA"}
