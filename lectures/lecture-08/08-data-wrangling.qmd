...
format:
  clean-revealjs:
    ...
    footer: "[Data Wrangling with Pandas](https://danilofreire.github.io/datasci151-summer/lectures/lecture-08/08-data-wrangling.html)"
revealjs-plugins:
  - multimodal
transition: slide
...
# Hello again! 🥳 {background-color="#4338CA"}

# Recap of last class 📚 {background-color="#4338CA"}
...
:::{.columns}
:::{.column width="50%"}
- How to define functions with `def` and `return`, and why they help us [avoid repeating code]{.alert}
- The difference between [parameters, arguments, and return values]{.alert}
- How to write [lambda functions](https://realpython.com/python-lambda/) for short, one-line tasks
- How [variable scope]{.alert} works: local variables live inside the function, global ones live outside
:::
...
:::{.columns}
:::{.column width="50%"}
- Introduction to [`pandas`]{.alert}, the [main library for data manipulation](https://pandas.pydata.org/) in Python
- Applying [functions]{.alert} to many variables at once with `apply` and `map`
- [Recoding and replacing]{.alert} variables in a dataset
- [`NaN` values]{.alert} ("Not a Number"): what they are and how to handle them
- Converting [variable types]{.alert} (e.g., string to numeric)
- Creating [new variables]{.alert} from existing ones
- [`.py` files]{.alert} and how to import them as modules
:::
...
# Operations over many variables using Pandas 🐼 {background-color="#4338CA"}
...
- It is a [method of a pandas DataFrame]{.alert}
  - A [method]{.alert} is a function that belongs to an object: you call it with the dot syntax (`object.method()`) and it acts on the object's data
...
:::{.columns}
:::{.column width="50%"}
- [Tab-completion]{.alert} is your daily tool. Type the name, a dot, then hit `Tab`:
...
- [`dir()`]{.alert} prints the full list as text
...
:::{.column width="50%"}
- [`?`]{.alert} opens the docstring for a method:
...
:::{style="margin-top: 30px; font-size: 24px;"}
- We can use `apply` itself to [build a brand-new column]{.alert} from an existing one: assign the result of `apply` to a new column name and pandas will add it to the DataFrame
...
- Most `pandas` methods are [non-destructive]{.alert}. `drop` does not actually remove the column from `data` on its own; it returns a [new DataFrame]{.alert} with that column removed
- To make the change stick, we must [reassign]{.alert} the result back to `data` (or pass `inplace=True`)
...
- Two other ways to delete a column (just so you know they exist):
...
:::{style="margin-top: 30px; font-size: 22px;"}
- `map` [applies a function]{.alert} to a list, an array, or a Series
  - A Series is one column of a DataFrame
- On a Series, `map` and `apply` are mostly [interchangeable]{.alert}
  - `map` is often faster; `apply` is more flexible (it also works on whole DataFrames)
- For [plain Python lists]{.alert}, use `map`, since `apply` is not a built-in Python function
- About `data[["age", "age_bracket01"]]`: the [outer brackets]{.alert} index the DataFrame, and the [inner brackets]{.alert} build a list of column names. Single brackets return a Series, double brackets return a DataFrame
...
# Importing modules 📦 {background-color="#4338CA"}
...
- [A Python script, or module, is just a file containing Python code]{.alert}
...
- However, [it is not recommended to import variables]{.alert}
- It is better to [import functions and use them to create variables]{.alert}
...
# Loading packages and dataset 📦 {background-color="#4338CA"}
...
:::{.columns}
:::{.column width="50%"}
[![](figures/codebook.png)](#){data-modal-type="image" data-modal-url="figures/codebook.png"}
...
- [Quick discussion]{.alert}: What does `varchar(255)` mean?
:::
...
# NaN values 🚫 {background-color="#4338CA"}
...
- It is a special value in Python that represents [missing data]{.alert}
...
# Data cleaning 🧹🧽 {background-color="#4338CA"}
...
- It involves [identifying and handling missing data, outliers]{.alert}, and other data quality issues
- [You have no idea]{.alert} how much time you will spend cleaning data in your life 😅
...
- 57% say it's the [least enjoyable]{.alert} part of their work
...
:::{.columns}
:::{.column width="50%"}
- We will use [`.str.isnumeric()`]{.alert}, which combines two calls:
  - [`.str`]{.alert}: treat the column as text
  - [`.isnumeric()`]{.alert}: check whether each string is numeric
- [Why do we need both?]{.alert} DataFrame columns often mix data types, so we have to treat the values as strings before we can test them
- Using `.isnumeric()` on its own would raise an error (trust me, I tried 😅)
- [`.str.isnumeric()`]{.alert} checks characters, not values: it rejects decimals (`"3.5"`), negatives (`"-2"`), and commas (`"1,000"`)
- For a true "is this a number?" test, use [`pd.to_numeric(s, errors="coerce").notna()`]{.alert}
:::

:::{.column width="50%"}
- The two dots between the calls are [method chaining]{.alert}: stacking several function calls in one line
...
:::{style="margin-top: 30px; font-size: 25px;"}
- [`query()`]{.alert} is a pandas DataFrame method that lets us [reference subattributes of a column](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.query.html) (we'll see more of it later)
- Combined with [`pd.unique()`]{.alert} (which returns the unique values of a column), we can extract every non-numeric value in one go
...
# Recoding numeric variables 🔄 {background-color="#4338CA"}
...
- Please remember to [convert the variable to the correct type]{.alert} before recoding
...
:::{style="margin-top: 30px; font-size: 25px;"}
- We can chop a numeric variable into [labelled intervals]{.alert} with `pd.cut()`
...
- `right = True` (default) makes intervals [right-closed]{.alert}: `(0, 2500]` includes 2500 but not 0
...
# Thanks very much! 😊 {background-color="#4338CA"}
