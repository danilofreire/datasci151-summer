...
- [In pandas]{.alert}, `map` works very similarly to `apply`, and they are interchangeable when working with series
- `map` can be faster for simple functions; `apply` is more flexible (it also works with DataFrames)
- For regular lists (e.g., `list01 = [1,2,3]`), use `map`, since [`apply` is not a built-in Python function]{.alert}
- When we ask for two or more columns at once, we use [double square brackets]{.alert} (e.g., `data[["age", "age_bracket01"]]`). The outer brackets are the indexing operator; the inner brackets build a [list of column names]{.alert} to pass to it. Single brackets with one name return a Series; double brackets with a list return a DataFrame
...
- The argument `right = True` (the default) means each interval is [right-closed, left-open]{.alert}: `(lower, upper]`. So `bins = [0, 2500, 5000]` gives `(0, 2500]` and `(2500, 5000]`, and a value of exactly 2500 falls in the first bin
  - Heads-up: a value of exactly 0 falls in [no bin]{.alert} and becomes `NaN`. Use `include_lowest = True` if you need the lower edge included
